...
def get_pcap_name():
    ...
    return os.path.join(diskpath, pcap_filename)
    ...
    while not event.is_set() or not sni.empty(): 
        if (size) and (os.path.exists(pcap_filename) and (os.path.getsize(pcap_filename)) > size):
            if rotate:
                last_pcap_time=time.time()
            process_pcap(False)
        if rotate and (os.path.exists(pcap_filename) and (time.time()-last_pcap_time)>rotate):
            last_pcap_time=time.time()
            process_pcap(False)
        ...
        if config.has_section('ZABBIX') or config.has_section('WEB'):
            ...
            if config.has_section('ZABBIX'):
                ...
                try:
                    ...
                    print(f"Error sending stadistics to Zabbix: {e}")
        else:
            print(stats_data)
        time.sleep(0.1)
